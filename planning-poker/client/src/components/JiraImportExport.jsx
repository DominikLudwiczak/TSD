...
const JiraImportExport = ({ sessionId, onImportSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/jira/import`, formData, {
        headers: {
  ...
  const handleExport = async () => {
    try {
      ...
      window.open(`${process.env.REACT_APP_BACKEND_URL}/api/jira/export/${sessionId}`, '_blank');
    } catch (err) {
