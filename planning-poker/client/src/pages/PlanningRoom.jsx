...
const PlanningRoom = ({ user }) => {
  ...
  const [participants, setParticipants] = useState([user.nickname]);

  // Tworzenie nowej sesji po wejściu do pokoju
  useEffect(() => {
    const startSession = async () => {
      const res = await createSession(user.roomId, user._id);
  ...
  const handleSelect = async (card) => {
    ...
    if (!sessionId) return alert('Brak ID sesji');

    await addEstimation(sessionId, user._id, card);

    setAllVotes((prev) => [
      ...prev.filter((v) => v.userId !== user._id),
      { userId: user._id, card },
    ]);
  };

  const handleReveal = async () => {
    if (!sessionId) return;
    await revealEstimations(sessionId);
    setShowCards(true);
  };
  ...
  const hasVoted = (userId) =>
    allVotes.some((v) => v.userId === userId);

  const allVoted = participants.every((u) => hasVoted(u));
      ...
      {showCards && (
        <>
          ...
          <ul>
            {allVotes.map((v, i) => (
              <li key={i}>
                {v.userId === user._id ? `${user.nickname} (Ty)` : v.userId}: {v.card}
              </li>
            ))}
          </ul>
        </>
